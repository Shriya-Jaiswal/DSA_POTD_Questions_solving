...
// ⏲️ Time Complexity : O(N * sqrt(N) * alpha) where alpha is for Union Find operations. But it's small , so we can ignore it to O(1) as average --> O(N*sqrt(N))
...
int main()
{
    ...
    bool ans = sol.canTraverseAllPairs(nums);
